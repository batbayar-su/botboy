module Lita
  module Handlers
    ...
    class AirQuality < Handler
      route(/air\squality\s(?<keyword>.*)/i, :aqi, command: false,
                              help: { 'air quality <address>' => "enter search word just after air quality.\nEx: air quality ulaanbaatar" })
      ...
      def aqi(response)
        ...
        # raise keyword.to_s
        if parsed_json['status'] != 'ok' or parsed_json['data'].length == 0
          ...
          average_air_quality = average_air_quality / parsed_json['data'].length
